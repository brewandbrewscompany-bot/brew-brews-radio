...
// Witch Ride 3D — Pass 15 restoration of the September 8 approved Pass 14 witch.
// The approved Pass 14 GLB, proportions, local stance and secondary motion are the visual source of truth.
const PASS_ID='witch-centerpiece-pass-v15';
const VERSION='pass-15-sept8-approved-restore-v1';
const REVIEW='september-8-pass14-approved-visual-lock';
const APPROVED_COMMIT='fc57a127836225f61a17f57e8af1540415dd6f18';
const APPROVED_ARTIFACT='witch-pass14-mesh-renders';
...
function captureStatic(witch,name){
  const node=witch.findByName(name);if(!node)return null;
  return {node,pos:node.getLocalPosition().clone(),euler:node.getLocalEulerAngles().clone(),scale:node.getLocalScale().clone()};
}
function restoreStatic(p){
  if(!p?.node)return;
  p.node.setLocalPosition(p.pos.x,p.pos.y,p.pos.z);
  p.node.setLocalEulerAngles(p.euler.x,p.euler.y,p.euler.z);
  p.node.setLocalScale(p.scale.x,p.scale.y,p.scale.z);
}
function setDelta(p,x,y,z){if(p?.node)p.node.setLocalEulerAngles(p.base.x+x,p.base.y+y,p.base.z+z)}
function snapshotStatic(stance){
  const out={};for(const [name,p] of Object.entries(stance))out[name]={position:[p.pos.x,p.pos.y,p.pos.z],euler:[p.euler.x,p.euler.y,p.euler.z],scale:[p.scale.x,p.scale.y,p.scale.z]};return out;
}
...
function install(){
  ...
  if(missing.length){console.error('Pass 15 September 8 restore roots missing:',missing.join(', '));return false}
  const stance={};for(const name of STANCE_NAMES)stance[name]=captureStatic(witch,name);
  if(Object.values(stance).some(x=>!x)){console.error('Pass 15 September 8 stance roots unavailable');return false}
  const stanceBaseline=snapshotStatic(stance);
  const lockGameplayPresentation=()=>{
    witch.setLocalScale(GAMEPLAY_SCALE,GAMEPLAY_SCALE,GAMEPLAY_SCALE);
    for(const p of Object.values(stance))restoreStatic(p);
  };
  ...
  window.WitchRideWitchCenterpiecePass={
    passId:PASS_ID,version:VERSION,review:REVIEW,approvedCommit:APPROVED_COMMIT,approvedArtifact:APPROVED_ARTIFACT,
    lockedMeshBlob:LOCKED_GLB_BLOB,active:true,visualOnly:true,meshOnly:true,materialsApplied:false,geometryLocked:true,
    gameplayScale:GAMEPLAY_SCALE,scaleEnforced:true,stanceLocked:true,stanceMode:'pass14-exact-local-transforms',stanceOffsetsRemoved:true,
    stanceBaseline,hatLocked:true,broomFlowAxis:'+Z toward chase camera/player',requiredNodes:REQUIRED.slice(),missing:[]
  };
  return true;
}
function boot(attempt=0){if(install())return;if(attempt<180)setTimeout(()=>boot(attempt+1),100);else console.error('Pass 15 September 8 approved witch restore did not initialize')}
boot();
