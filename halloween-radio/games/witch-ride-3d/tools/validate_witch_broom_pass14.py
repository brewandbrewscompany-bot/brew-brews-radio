...
assert meta['broom_geometry_workflow_version'] == 'hierarchy-v3-core-ribbons'
...
for name, mesh in zip(primary_names, primary_meshes):
    ...
    assert float(mesh.vertices[:, 1].min()) > -2.55, (name, mesh.vertices[:, 1].min())
...
for name, mesh in zip(straw_names, straw_meshes):
    ...
    tip = mesh.vertices[mesh.vertices[:, 1] <= ymin + 0.045]
    assert len(tip) >= 4, (name, len(tip))
    assert float(np.ptp(tip[:, 0])) <= 0.020, (name, np.ptp(tip[:, 0]))
    assert float(np.ptp(tip[:, 2])) <= 0.016, (name, np.ptp(tip[:, 2]))
...
# Primary core must build a real packed body and stop before the visible terminal third.
assert primary_span[0] >= 1.85, primary_span
assert primary_span[1] >= 1.95, primary_span
assert primary_span[2] >= 1.25, primary_span
assert float(primary[:, 1].min()) > -2.55, primary[:, 1].min()

# Secondary ribbons and fine bristles carry the long irregular tail.
assert straw_span[0] >= 2.55, straw_span
assert straw_span[1] >= 3.85, straw_span
assert straw_span[2] >= 1.45, straw_span
assert bristle_span[0] >= 2.45, bristle_span
assert bristle_span[1] >= 3.70, bristle_span
assert bristle_span[2] >= 1.40, bristle_span
...
# Six terminal length families break the bottom edge. No even comb line is allowed.
length_targets = np.array([-2.92, -3.18, -3.47, -3.78, -4.08, -4.40], dtype=float)
clump_low_y = np.asarray([float(m.vertices[:, 1].min()) for m in straw_meshes])
...
assert float(np.ptp(clump_low_y)) >= 1.35, np.ptp(clump_low_y)
assert float(np.std(clump_low_y)) >= 0.43, np.std(clump_low_y)

# Secondary clumps must fill a continuous width instead of collapsing into eight branch axes.
tip_x = []
...
full_root_count = sum(float(m.vertices[:, 1].max()) > -0.40 for m in bristle_meshes)
assert 45 <= full_root_count <= 51, full_root_count

# Large tail silhouette remains broad without overwhelming the rider root zone.
tail = np.vstack([straw[straw[:, 1] <= -2.70], bristles[bristles[:, 1] <= -2.70]])
