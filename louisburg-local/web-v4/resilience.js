...
(function(){
  ...
  function cacheFeed(items){
    const clean=townItems(items);if(!validItems(clean)||!clean.length)return;
    cachedFeedTimestamp=Date.now();storageSet(localStorage,FEED_KEY,{version:VERSION,ts:cachedFeedTimestamp,items:clean});
  }
  function cacheRegistry(items){
    if(!validRegistry(items)||!items.length)return;
    cachedRegistryTimestamp=Date.now();storageSet(localStorage,REGISTRY_KEY,{version:VERSION,ts:cachedRegistryTimestamp,items});
  }
  ...
  if(cachedFeed&&validItems(cachedFeed.items)&&cachedFeed.items.length){
    cachedFeedTimestamp=Number(cachedFeed.ts||0);
    if(state&&(!Array.isArray(state.items)||!state.items.length)){
      restoring=true;
      state.items=cachedFeed.items.slice();
      renderTownViews();
      restoring=false;
  ...
  if(cachedRegistry&&validRegistry(cachedRegistry.items)&&cachedRegistry.items.length){
    cachedRegistryTimestamp=Number(cachedRegistry.ts||0);
  ...
  if(typeof renderHome==='function'){
    ...
    renderHome=function(){
      ...
      if(!restoring&&state&&Array.isArray(state.items))cacheFeed(state.items);
      return out;
    };
  }
  if(typeof renderDirectory==='function'){
    ...
    renderDirectory=function(){
      ...
      if(!restoring&&state&&Array.isArray(state.registry)&&state.registry.length)cacheRegistry(state.registry);
      return out;
    };
  }
  ...
  // Replace subsequent API calls with an abortable equivalent. The initial inline-page request may already be in flight.
  ...
  function applyTownFeed(items,quiet){
    ...
    const clean=townItems(items);
    if(!clean.length&&currentTownSignature())return false;
    if(signature(clean)===currentTownSignature()){
      if(clean.length)cacheFeed(clean);
    ...
    if(state&&state.screen==='wildcats'){
      pendingTownItems=clean.slice();
      window.__llV4PendingTownItems=pendingTownItems;
      cacheFeed(clean);
      return true;
    }
    state.items=clean.slice();cacheFeed(clean);renderTownViews();
    if(!quiet)setFeedStatus(`live · ${clean.filter(i=>typeof fresh==='function'?fresh(i):true).length} current`);
    return true;
  }
  ...
  async function refreshRegistry(){
    ...
    try{
      ...
      if(signature(items)!==signature(current)){
        state.registry=items;rebuildRegistryMap();cacheRegistry(items);
        if(state.screen==='directory'&&typeof renderDirectory==='function')renderDirectory();
      }else cacheRegistry(items);
      return true;
  ...
  function saveUiState(){
    ...
    captureWildcatUi(out);
    storageSet(sessionStorage,UI_KEY,out);
  }
  ...
  function restoreWildcatUi(){
    ...
    const wantedView=savedUi.wildcatView,wantedFilter=savedUi.wildcatFilter,wantedTown=savedUi.wildcatTown;
    const view=wantedView&&document.querySelector(`[data-wt-view="${CSS.escape(wantedView)}"]`);if(view&&!view.classList.contains('active'))view.click();
    const filter=wantedFilter&&document.querySelector(`[data-wt-filter="${CSS.escape(wantedFilter)}"]`);if(filter&&!filter.classList.contains('active'))filter.click();
    const town=document.querySelector('#wildcatTownToggle');if(town&&typeof wantedTown==='boolean'&&((town.getAttribute('aria-pressed')==='true')!==wantedTown))town.click();
    const first=document.querySelector('#wildcatCalendar [data-wt-date]');
    if(first&&savedUi.wildcatMonth){
      const current=String(first.dataset.wtDate||'').slice(0,7),delta=monthDelta(current,savedUi.wildcatMonth);
      const btn=delta<0?document.querySelector('#wildcatPrevMonth'):document.querySelector('#wildcatNextMonth');
      for(let n=0;n<Math.min(18,Math.abs(delta));n++)if(btn)btn.click();
    }
    if(savedUi.wildcatSelected){const d=document.querySelector(`#wildcatCalendar [data-wt-date="${CSS.escape(savedUi.wildcatSelected)}"]`);if(d)d.click()}
    return true;
  }
  ...
  function restoreUi(){
    syncSelections();
    const wanted=savedUi.screen;
    const screen=wanted&&document.querySelector(`#${CSS.escape(wanted)}Screen`);
    if(wanted&&screen&&typeof showScreen==='function'){
      showScreen(wanted);
      if(wanted==='wildcats')restoreWildcatUi();
      setTimeout(()=>window.scrollTo({top:Number(savedUi.scrollY||0),left:0,behavior:'auto'}),60);
      return;
    }
    if(wanted==='wildcats'&&restoreAttempts++<30){setTimeout(restoreUi,150);return}
    if(savedUi.scrollY)setTimeout(()=>window.scrollTo({top:Number(savedUi.scrollY||0),left:0,behavior:'auto'}),60);
  }
  ...
  window.addEventListener('offline',()=>{
    setFeedStatus(currentTownSignature()?`offline · ${ageText(cachedFeedTimestamp)}`:'offline');
  });
  window.addEventListener('online',()=>{
    setFeedStatus(currentTownSignature()?`${ageText(cachedFeedTimestamp)} · updating…`:'reconnecting…');
    refreshFeed('online');refreshRegistry();
  });

  // If the original startup request is still unresolved, the cached copy remains visible and we start an abortable retry.
  setTimeout(()=>{
  ...
  periodicTimer=setInterval(()=>{
    ...
    refreshFeed('periodic');refreshRegistry();
  },PERIODIC_MS);
