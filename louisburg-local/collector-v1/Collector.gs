...
function runCollectorV22SelfTest() {
  ...
  const cases = [
    ...
    {name:'mixed stale explicit year', text:'Louisburg KS. Car show May 4, 2025 at 11:00 am. Club meeting September 28 at 8:00 am.', want:false},
    {name:'facility reservation shell', text:'Louisburg KS. Fox Hall reservations. Fox Hall reserved August 31, 2026, 2:45 pm - 5:15 pm. Ron Weers Park shelter reserved.', want:false},
    {name:'broad lineup shell', text:'Louisburg KS. Upcoming events 2026-06-14T02:00:00.000Z - 2026-12-19T06:00:00.000Z upcoming live music line up. Contact us today.', want:false},
    {name:'distant yearless seasonal', text:'Louisburg KS. Seasonal events and workshops. Our very first season begins November 20.', want:false},
    ...
    {name:'near term yearless event', text:'Louisburg KS. News and events. Outdoor flea market October 10 at 9:00 am. Details to be announced.', want:true},
    {name:'ciderfest explicit date', text:'Louisburg KS. Ciderfest September 26, 2026, 8:00 am - 6:00 pm at the Louisburg Cider Mill.', want:true}
  ];
  const failures = [];
  ...
  try {
    ...
    while (visited < rows.length && considered < maxEndpoints) {
      ...
      try {
        ...
        let activity, gate;
        const now=new Date();
        if (access==='SURFACE' && !social.readable) {
          activity={signals:[],snippets:[],canonical:'',contentDate:'',qualityScore:0,dateDriven:false};
          gate={ok:false,reason:'CONTEXT',label:'FILTERED UNREADABLE SOCIAL SURFACE'};
        } else {
          activity=extractActivity_(normalized);
          gate=activityQualityGate_(activity,normalized,org,now);
        }
...
function activityQualityGate_(activity,text,org,now) {
  ...
  if(looksLikeCode_(c))return {ok:false,reason:'NOISE',label:'FILTERED CODE/BOILERPLATE'};
  if(isFacilityReservationShell_(c))return {ok:false,reason:'CONTEXT',label:'FILTERED FACILITY RESERVATION'};
  if(isBroadLineupShell_(c))return {ok:false,reason:'CONTEXT',label:'FILTERED BROAD LINEUP SHELL'};
  ...
  if(dates.explicitPastOnly&&!hasCurrent&&!hasOperational)return {ok:false,reason:'STALE',label:'FILTERED EXPLICIT STALE YEAR(S)'};
  if(dates.pastOnly)return {ok:false,reason:'STALE',label:'FILTERED PAST DATE(S)'};
  if(dates.onlyInferred&&dates.nearestFutureDays!=null&&dates.nearestFutureDays>60&&!hasCurrent&&!hasFreshProduct&&!hasOperational)return {ok:false,reason:'CONTEXT',label:'FILTERED DISTANT YEARLESS DATE'};
  ...
  score+=Math.min(strong.length,3)*2;
  if(dates.hasCurrentOrFuture)score+=4;
  if(dates.explicitHasCurrentOrFuture)score+=2;
  if(hasTime)score+=2;
  if(hasCurrent)score+=4;
  if(hasFreshProduct)score+=3;
  if(hasOperational)score+=3;
  if(activity.dateDriven)score+=2;
  const evergreen=LL_CONFIG.EVERGREEN_TERMS.filter(function(x){return c.indexOf(x)>=0;}).length;
  if(evergreen>=2&&!dates.hasCurrentOrFuture&&!hasCurrent&&!hasFreshProduct)score-=4;
...
function snippetContextGate_(snippet) {
  ...
  if(!s||s.length<35)return {ok:false,reason:'CONTEXT'};
  if(looksLikeCode_(s))return {ok:false,reason:'NOISE'};
  ...
  const nav=LL_CONFIG.NAVIGATION_TERMS.filter(function(x){return s.indexOf(x)>=0;}).length;
  const date=containsDateLike_(s), time=/\b\d{1,2}:\d{2}\s*(am|pm)\b|\b\d{1,2}\s*(am|pm)\b/.test(s), current=/\b(today|tonight|tomorrow|this weekend|limited time|available now|closed today|now hiring|registration open|register now)\b/.test(s);
...
function extractActivity_(text) {
  ...
  function addAt_(pos,len,fromDate){
    const start=Math.max(0,pos-LL_CONFIG.ACTIVITY_SNIPPET_BEFORE),end=Math.min(source.length,pos+len+LL_CONFIG.ACTIVITY_SNIPPET_AFTER);
    const snippet=source.slice(start,end).replace(/\s+/g,' ').trim(),compact=snippet.replace(/[^a-z0-9]+/g,' ').trim();
    if(snippet.length<35||!compact)return;
    const key=digest_(compact).slice(0,16);
    if(!seen[key]&&snippets.length<LL_CONFIG.ACTIVITY_MAX_SNIPPETS){seen[key]=true;snippets.push(snippet);if(fromDate)dateDriven=true;}
  }
  ...
  const dateRe=/\b20\d{2}[-\/][01]?\d[-\/][0-3]?\d\b|\b(?:january|february|march|april|may|june|july|august|september|october|november|december|jan|feb|mar|apr|jun|jul|aug|sep|sept|oct|nov|dec)\.?\s+\d{1,2}(?:st|nd|rd|th)?(?:,?\s+20\d{2})?\b/gi;
  let m,dateHits=0;
  while((m=dateRe.exec(source))&&dateHits<12&&snippets.length<LL_CONFIG.ACTIVITY_MAX_SNIPPETS){
    const probe=source.slice(Math.max(0,m.index-90),Math.min(source.length,m.index+m[0].length+140));
    if(looksDateActivityLike_(probe))addAt_(m.index,m[0].length,true);
    dateHits++;
  }
  snippets.sort();
  return {signals:uniqueStrings_(signals),snippets:snippets,canonical:snippets.join('\n'),contentDate:'',qualityScore:0,dateDriven:dateDriven};
}

function looksDateActivityLike_(s){
  const t=String(s||'').toLowerCase();
  return /\b\d{1,2}:\d{2}\s*(am|pm)\b|\b\d{1,2}\s*(am|pm)\b/.test(t)||/\b(event|meeting|class|workshop|concert|music|game|market|festival|sale|special|register|registration|closed|closure|opening|open house|dominoes|storytime|tour|practice|tryout|fundraiser|reserved|reservation)\b/.test(t);
}

function analyzeActivityDates_(text,now){
  const s=String(text||''), explicit=[], inferred=[], currentYear=Number(Utilities.formatDate(now,LL_CONFIG.TZ,'yyyy'));
  let m;
  const iso=/\b(20\d{2})[-\/]([01]?\d)[-\/]([0-3]?\d)\b/g;
  while((m=iso.exec(s)))add_(explicit,Number(m[1]),Number(m[2])-1,Number(m[3]));
  const mon=/\b(january|february|march|april|may|june|july|august|september|october|november|december|jan|feb|mar|apr|jun|jul|aug|sep|sept|oct|nov|dec)\.?\s+(\d{1,2})(?:st|nd|rd|th)?(?:,?\s+(20\d{2}))?\b/gi;
  ...
  while((m=mon.exec(s))){
    if(m[3])add_(explicit,Number(m[3]),months[m[1].toLowerCase()],Number(m[2]));
    else add_(inferred,currentYear,months[m[1].toLowerCase()],Number(m[2]));
  }
  function add_(arr,y,mo,d){const x=new Date(y,mo,d,12,0,0);if(!isNaN(x.getTime()))arr.push(x);}
  const dates=explicit.concat(inferred);
  if(!dates.length)return {found:false,hasCurrentOrFuture:false,pastOnly:false,bestIso:'',explicitFound:false,explicitHasCurrentOrFuture:false,explicitPastOnly:false,onlyInferred:false,nearestFutureDays:null};
  const today=new Date(now.getFullYear(),now.getMonth(),now.getDate(),12,0,0);
  function summary_(arr){
    let future=false,past=false,best=null,bestMs=Infinity,nearestFutureDays=null;
    arr.forEach(function(x){
      const delta=x.getTime()-today.getTime();
      if(delta>=0){
        future=true;
        const days=Math.round(delta/86400000);
        if(nearestFutureDays==null||days<nearestFutureDays)nearestFutureDays=days;
        if(delta<bestMs){bestMs=delta;best=x;}
      }else{
        past=true;
        if(!best&&Math.abs(delta)<bestMs){bestMs=Math.abs(delta);best=x;}
      }
    });
    return {future:future,past:past,best:best,nearestFutureDays:nearestFutureDays};
  }
  const all=summary_(dates), ex=summary_(explicit);
  return {
    found:true,
    hasCurrentOrFuture:all.future,
    pastOnly:all.past&&!all.future,
    bestIso:all.best?Utilities.formatDate(all.best,LL_CONFIG.TZ,'yyyy-MM-dd'):'',
    explicitFound:explicit.length>0,
    explicitHasCurrentOrFuture:ex.future,
    explicitPastOnly:explicit.length>0&&ex.past&&!ex.future,
    onlyInferred:explicit.length===0&&inferred.length>0,
    nearestFutureDays:all.nearestFutureDays
  };
}

function containsDateLike_(s){
  return /\b20\d{2}[-\/][01]?\d[-\/][0-3]?\d\b|\b(?:january|february|march|april|may|june|july|august|september|october|november|december|jan|feb|mar|apr|jun|jul|aug|sep|sept|oct|nov|dec)\.?\s+\d{1,2}(?:st|nd|rd|th)?(?:,?\s+20\d{2})?\b/i.test(String(s||''));
}

function isFacilityReservationShell_(s){
  const t=String(s||'').toLowerCase();
  return /\b(reserved|reservation|reservations)\b/.test(t)&&/(fox hall|park shelter|ron weers park|shelter- reserved|shelter reserved)/.test(t)&&!/\b(public meeting|festival|concert|class|workshop|registration|register|storytime|fundraiser|open house)\b/.test(t);
}

function isBroadLineupShell_(s){
  const t=String(s||'').toLowerCase();
  if(!/upcoming live music line ?up|live music line ?up/.test(t))return false;
  const hasHumanEvent=/\b(?:jan|feb|mar|apr|may|jun|jul|aug|sep|sept|oct|nov|dec|january|february|march|april|june|july|august|september|october|november|december)\.?\s+\d{1,2}[^|]{0,80}\b\d{1,2}(?::\d{2})?\s*(am|pm)\b/i.test(t);
  return !hasHumanEvent;
}

function looksLikeCode_(s){
  const t=String(s||'').toLowerCase();
  const strong=(t.match(/siteassetstestmoduleversion|wixdevelopersanalytics|xsrftoken|specs\.thunderbolt|specs\.odeditor|viewerpublicsegmentsiteassetstemplate|polaris\.(loginpage|postpage|profilepage)|text-badge-[a-z-]+-background|appdefid|instanceid/g)||[]).length;
  const hits=(t.match(/pointer-events|font-size|background-color|artifactid|metasiteid|window\.|function\s|requesturl/g)||[]).length;
  const p=(t.match(/[{};<>]/g)||[]).length,long=(t.match(/\b[a-z0-9_-]{80,}\b/g)||[]).length;
  return strong>=1||hits>=2||long>=2||p>Math.max(20,t.length*.025);
}

function inspectSocialSurface_(url,text,html){
  const u=String(url||'').toLowerCase(),fb=/facebook\.com|fb\.com/.test(u),ig=/instagram\.com/.test(u);
  if(!fb&&!ig)return {readable:true,status:'DIRECT READABLE'};
  const h=String(text||'').toLowerCase(),raw=String(html||'').toLowerCase(),wall=/log in|login|create new account|sign up|accounts\/login|checkpoint/.test(h+' '+raw),post=/posted|photos|reel|followers|likes|comments|instagram|facebook/.test(h),meaningful=h.length>500&&post&&!wall;
  if(meaningful)return {readable:true,status:(fb?'FACEBOOK':'INSTAGRAM')+' SURFACE READABLE'};
  if(wall)return {readable:false,status:(fb?'FACEBOOK':'INSTAGRAM')+' LOGIN/INTERSTITIAL'};
  return {readable:false,status:(fb?'FACEBOOK':'INSTAGRAM')+' SURFACE LIMITED'};
}
...
function processSherlockRechecks_(ss){
  ...
  for(let r=0;r<data.length&&processed<LL_CONFIG.SHERLOCK_RECHECK_LIMIT_PER_RUN;r++){
    ...
    if(sourceUrl&&/^https?:\/\//i.test(sourceUrl)){
      try{
        const fetched=fetchPublicSource_(sourceUrl),text=normalizeText_(fetched.body),activity=extractActivity_(text),gate=activityQualityGate_(activity,text,'',new Date());
        resultText='HTTP '+fetched.status+'; '+gate.label+'; current signals: '+(activity.signals.join(', ')||'none detected');
...
function fetchPublicSource_(url){
  ...
  for(let attempt=0;attempt<=LL_CONFIG.RETRY_COUNT;attempt++){
    try{
      const response=UrlFetchApp.fetch(url,{muteHttpExceptions:true,followRedirects:true,headers:{'User-Agent':LL_CONFIG.USER_AGENT,'Accept':'text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8'}}),status=response.getResponseCode();
...
function normalizeText_(html){
  return String(html||'').replace(/<script[\s\S]*?<\/script>/gi,' ').replace(/<style[\s\S]*?<\/style>/gi,' ').replace(/<!--([\s\S]*?)-->/g,' ').replace(/<[^>]+>/g,' ').replace(/&nbsp;/gi,' ').replace(/&amp;/gi,'&').replace(/&#39;/g,"'").replace(/&quot;/gi,'"').replace(/&#x27;/gi,"'").replace(/&#x2f;/gi,'/').replace(/\s+/g,' ').trim().toLowerCase();
}
...
function appendVerificationCandidate_(ss,org,url,activity,fingerprint,now){
  ...
  sheet.appendRow([org,'Activity change candidate',preview||('Signals: '+activity.signals.join(', ')),'Verify exact new activity, date, Louisburg applicability and original destination before publication',url,'MEDIUM','OPEN - COLLECTOR',Utilities.formatDate(now,LL_CONFIG.TZ,'yyyy-MM-dd'),'','Activity fingerprint '+fingerprint.slice(0,16)+'; quality='+activity.qualityScore+'; signals: '+activity.signals.join(', ')+'. V2.2 contextual filter passed. Content-level source must beat business-level source.']);
}
function verificationCandidateExists_(ss,org,url,fingerprint){
  const sheet=ss.getSheetByName(LL_CONFIG.SHEETS.VERIFY);
  if(!sheet||sheet.getLastRow()<2)return false;
  const data=sheet.getDataRange().getDisplayValues(),needle=String(fingerprint||'').slice(0,16);
  for(let i=1;i<data.length;i++){
    if(String(data[i][0]).trim()===org&&String(data[i][4]).indexOf(url)!==-1&&/^OPEN/.test(String(data[i][6]).toUpperCase())&&(!needle||String(data[i][9]).indexOf(needle)!==-1))return true;
  }
  return false;
}
function loadState_(sheet){
  ...
  for(let r=1;r<data.length;r++){
    ...
    out[data[r][0]]={row:r+1,fingerprint:data[r][4],lastContentDate:data[r][5],lastChange:data[r][6],lastSuccess:data[r][8],nextCheck:data[r][9],failures:data[r][10],boostUntil:data[r][11]};
  }
  return out;
}
